...
def screen_un(name, entity_type):
        ...
        if res:
            ...
            if score >= threshold:
                if best_match is None or score > best_match["match_score"]:
                    ...
                    best_match = {
                        "match_found": True,
                        "match_type": "UN_CONSOLIDATED",
                        "matched_name": entry.get("name"),
                        "matched_alias": matched_str if is_alias else "",
                        "match_score": round(score, 2),
                        "list_name": _LIST_DATA.get("list_name", "UN Consolidated List"),
                        "regulatory_basis": _LIST_DATA.get("regulatory_basis", "UN Security Council"),
